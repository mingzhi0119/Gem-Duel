...
const resolvePort = async () => {
    if (process.env.GEM_DUEL_VISUAL_PORT) {
        return Number(process.env.GEM_DUEL_VISUAL_PORT);
    }
    ...
    return await new Promise((resolve, reject) => {
        ...
        server.listen(0, HOST, () => {
            ...
            server.close(() => reject(new Error('Unable to resolve a free port for check-visual')));
        });
    });
};
