...
import { getAllowedCommands, validateDispatch } from '@gem-duel/core-engine';
import {
    getBoardTokenPrompt,
    getBonusColorPrompt,
...
export const getNonGoldBoardCells = (snapshot: GameSnapshot) =>
    snapshot.board.filter((cell) => cell.token !== null && cell.token !== 'gold');

export const getGoldBoardCell = (snapshot: GameSnapshot) =>
    snapshot.board.find((cell) => cell.token === 'gold') ?? null;

export const getReserveSources = (snapshot: GameSnapshot) => [
...
export const getBuySources = (snapshot: GameSnapshot) => [
    ...snapshot.pyramid.flatMap((row) =>
        row.slots
            .filter((slot) => slot.card !== null)
            .map((slot) => ({
...
export const buildActions = (snapshot: GameSnapshot): UiActionDescriptor[] => {
