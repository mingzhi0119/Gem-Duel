...
export const AiTraceDrawer = ({
    ...
    if (traces.length === 0) {
        return (
            <SidecarDrawer
                title="AI Trace"
                mode="drawer"
                triggerSummary="No deterministic decisions yet"
                triggerTestId="ai-trace-drawer-trigger"
                panelTestId="ai-trace-drawer"
                size="wide"
                openLabel={drawerMessages.openLabel}
                closeLabel={drawerMessages.closeLabel}
            >
                <p className="gd-muted">
                    The AI has not made a deterministic decision in this scene.
                </p>
            </SidecarDrawer>
        );
    }
    ...
    return (
        <SidecarDrawer
            title="AI Trace"
            mode="drawer"
            triggerSummary={`${traces.length} deterministic decisions`}
            triggerBadge={<span className="gd-shell-badge">{traces.length}</span>}
            triggerTestId="ai-trace-drawer-trigger"
            panelTestId="ai-trace-drawer"
            size="wide"
            openLabel={drawerMessages.openLabel}
