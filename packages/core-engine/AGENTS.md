...
- Step 02 guardrails actively ban `Math.random`, `Date.now`, `new Date`, `performance.now`, hidden random libraries, and host IO imports in this package. Step 02 护栏已主动禁止本目录使用 `Math.random`、`Date.now`、`new Date`、`performance.now`、隐藏随机库与宿主 IO import。
