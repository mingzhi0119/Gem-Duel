...
test('Phase 6 row 1: spectator stream renders BoardScene but stays non-interactive', async ({
    ...
    try {
        ...
        await expect(spectator.locator('[data-testid="selection-confirm"]')).toHaveCount(0);
        await expect(spectator.locator('[data-testid="selection-cancel"]')).toHaveCount(0);

        await p2.getByRole('button', { name: 'Take gems' }).click();
        await expect
