...
- Keep imports limited to `@gem-duel/application`, `@gem-duel/contracts`, and `@gem-duel/adapters`; direct imports from `@gem-duel/core-engine` or `@gem-duel/domain` are outside the frozen Step 02 boundary. 本目录的 workspace import 仅限 `@gem-duel/application`、`@gem-duel/contracts` 与 `@gem-duel/adapters`；直接引入 `@gem-duel/core-engine` 或 `@gem-duel/domain` 超出 Step 02 冻结边界。
